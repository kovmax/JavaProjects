...
public class ParametrizedTest extends TestTemplate {
    ...
    @Test
    public void test1() {
        Double[] result = sort.sort(input, DOUBLE_COMPARATOR_1);
        Assert.assertTrue(testOrder(result, DOUBLE_COMPARATOR_1));
        Assert.assertEquals("Result array length should be equal to original", input.length, result.length);
        Assert.assertTrue(hasEachElementOf(input, result, DOUBLE_COMPARATOR_1));
    }

    @Test
    public void test2() {
        Double[] result = sort.sort(input, DOUBLE_COMPARATOR_2);
        Assert.assertTrue(testOrder(result, DOUBLE_COMPARATOR_2));
        Assert.assertEquals("Result array length should be equal to original", input.length, result.length);
        Assert.assertTrue(hasEachElementOf(input, result, DOUBLE_COMPARATOR_2));
    }
}
